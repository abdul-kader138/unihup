<?php

namespace Tests\Feature;

use App\Filament\Pages\CostEstimator;
use App\Models\DegreeProgram;
use App\Models\ShortlistItem;
use App\Models\Subject;
use App\Models\University;
use App\Models\User;
use Database\Seeders\ShieldSeeder;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Support\Str;
use Livewire\Livewire;
use Tests\TestCase;

class CostEstimatorPageTest extends TestCase
{
    use RefreshDatabase;

    protected function setUp(): void
    {
        parent::setUp();
        $this->seed(ShieldSeeder::class);
    }

    private function studentWithProgram(): array
    {
        $user = User::factory()->create();
        $user->assignRole('panel_user');

        $subject = Subject::firstOrCreate(['slug' => 'eng'], ['name' => 'Engineering']);
        $slug = 'u-'.Str::random(8);
        $university = University::create(['name' => 'U '.$slug, 'slug' => $slug, 'city' => 'Milan', 'region' => 'Lombardy']);
        $program = DegreeProgram::create([
            'university_id' => $university->id, 'subject_id' => $subject->id, 'degree_level' => 'master',
            'name' => 'MSc Data', 'language' => 'English', 'duration_years' => 2, 'admission_type' => 'open',
            'tuition_min' => 900, 'tuition_max' => 2800,
        ]);
        ShortlistItem::create(['user_id' => $user->id, 'degree_program_id' => $program->id, 'status' => 'researching']);

        return [$user, $program];
    }

    public function test_panel_user_can_open_the_page(): void
    {
        $user = User::factory()->create();
        $user->assignRole('panel_user');

        $this->actingAs($user)->get('/cost-estimator')->assertOk();
    }

    public function test_it_estimates_for_a_shortlisted_program(): void
    {
        [$user, $program] = $this->studentWithProgram();

        $result = Livewire::actingAs($user)
            ->test(CostEstimator::class)
            ->set('data.program_id', $program->id)
            ->set('data.isee', 20000)
            ->set('data.housing', 'shared')
            ->instance()
            ->getResult();

        $this->assertNotNull($result);
        $this->assertSame($program->id, $result['program']->id);
        $this->assertGreaterThan(0, $result['estimate']['net_max']);
        $this->assertSame('program', $result['estimate']['tuition']['source']);
    }

    public function test_it_refuses_a_program_the_user_has_not_shortlisted(): void
    {
        [$user] = $this->studentWithProgram();

        $other = DegreeProgram::create([
            'university_id' => University::first()->id,
            'subject_id' => Subject::first()->id,
            'degree_level' => 'bachelor', 'name' => 'Other', 'language' => 'English',
            'duration_years' => 3, 'admission_type' => 'open',
        ]);

        $result = Livewire::actingAs($user)
            ->test(CostEstimator::class)
            ->set('data.program_id', $other->id)
            ->instance()
            ->getResult();

        $this->assertNull($result);
    }
}
